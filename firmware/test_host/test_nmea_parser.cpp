...
TEST_CASE("GPRMC: latitude decoded correctly") {
    ...
    CHECK(approx(p.fix().lat_deg, 43.8130));
    CHECK(p.fix().lat_deg > 0.0);  // Northern hemisphere.
}
...
TEST_CASE("GPRMC: longitude decoded correctly (West = negative)") {
    ...
    CHECK(approx(p.fix().lon_deg, -79.3944));
    CHECK(p.fix().lon_deg < 0.0);
}
...
TEST_CASE("GPRMC: South latitude is negative") {
    ...
    p.process(s.c_str());
    CHECK(p.fix().lat_deg < 0.0);
    CHECK(approx(p.fix().lat_deg, -33.0));
}

TEST_CASE("GPRMC: speed converted from knots to km/h") {
    NmeaParser p;
    p.process(kRmcValid.c_str());
    // 6.0 knots × 1.852 = 11.112 km/h
    CHECK(approx(static_cast<double>(p.fix().speed_kmh), 6.0 * 1.852, 0.01));
}
...
TEST_CASE("GPRMC: unix timestamp for 1994-03-23 12:35:19 UTC") {
    ...
    CHECK(p.fix().timestamp_s == 764426119u);
}

TEST_CASE("GPRMC: timestamp is 0 before any valid fix") {
    NmeaParser p;
    CHECK(p.fix().timestamp_s == 0u);
}
...
TEST_CASE("GPRMC: unix timestamp for 2000-01-01 00:00:00 UTC") {
    ...
    p.process(s.c_str());
    CHECK(p.fix().timestamp_s == 946684800u);
}
...
TEST_CASE("GPGGA: satellite count decoded") {
    NmeaParser p;
    p.process(kGgaValid.c_str());
    CHECK(p.fix().sats_used == 8);
}

TEST_CASE("GPGGA: fix quality 1 stored as 1") {
    NmeaParser p;
    p.process(kGgaValid.c_str());
    CHECK(p.fix().fix_quality == 1);
}
...
TEST_CASE("mark_stale: fix data readable after stale") {
    ...
    p.process(kRmcValid.c_str());
    p.mark_stale();
    CHECK(approx(p.fix().lat_deg, 43.8130));  // Coordinates still available.
}
...
TEST_CASE("feed: second valid sentence updates fix") {
    ...
    p.feed('\r'); p.feed('\n');
    CHECK(p.valid());
    double lat1 = p.fix().lat_deg;

    // Feed the GGA sentence (also valid).
    for (char c : kGgaValid) p.feed(static_cast<uint8_t>(c));
    p.feed('\r'); p.feed('\n');
    CHECK(p.valid());
    CHECK(approx(p.fix().lat_deg, lat1));  // Same position in both sentences.
}
