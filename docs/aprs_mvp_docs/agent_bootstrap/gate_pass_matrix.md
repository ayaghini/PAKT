...
| Native app protocol drift between desktop and iPhone clients | Medium | Firmware now exposes a dedicated debug stream and richer device status. `05_ble_gatt_spec.md`, `19_ble_integration_reference.md`, `agent_bootstrap/ble_protocol_quickref.md`, `payload_contracts.md`, `app/desktop_test/`, and `app/ios/` must stay aligned as the iPhone app lands. | Firmware+Apps / ongoing |
