...
**Hardware-progressed but still open:** Step 1 now has reported SA818 UART/PTT plus supervised RF tone-TX and short APRS packet-TX evidence, and Step 2 now has reported live codec/radio audio-path evidence via TX tones and RX signal presence.
**Hardware-blocked:** Calibrated deviation measurement, on-device APRS RX decode proof, stored RX audio capture, full end-to-end validation of Steps 3–8, G1, G2(endurance), G3(hardware), G4; third-party KISS client validation
