...
|---|---|---|---|---|---|
| `esp32-s3` | `RF_Module.kicad_sym` | `ESP32-S3-WROOM-1.kicad_mod` | `ESP32-S3-WROOM-1.step` | provisional | Use as a module reference only after matching the exact module variant for the PCB. |
...
| `gps-backup-battery` | standard KiCad (`Device:Battery_Cell`, `Device:D_Schottky`) | selected during schematic (`BR-1225A/FAN` tab pattern + SOD-323 diode) | n/a | provisional | Backup battery path is now part of rev01 plan; verify exact battery tab footprint against chosen cell. |
| `max17048` | `MAX17048.kicad_sym` | `DFN-8-1EP_3x2mm_P0.5mm_EP1.7x1.4mm.kicad_mod` | `DFN-8-1EP_3x2mm_P0.5mm_EP1.75x1.45mm.step` | provisional | Dedicated local symbol added; confirm package drawing against exact orderable suffix before release. |
...
4. Verify the MAX17048 and MCP73831 package land patterns against selected manufacturers' package drawings before fab release.
5. Verify `BR-1225A/FAN` tab footprint land dimensions and polarity marking before PCB release.
